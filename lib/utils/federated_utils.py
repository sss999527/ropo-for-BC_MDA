...
def knowledge_vote(knowledge_list, confidence_gate, num_classes):     # [128,4,10],0.9,10
    """
    ...
    """
    max_p, max_p_class = knowledge_list.max(2)    # [128,4],[128,4]
    max_conf, _ = max_p.max(1)   # 128,128
    max_p_mask = (max_p > confidence_gate).float().cuda()   # [128,4]
    consensus_knowledge = torch.zeros(knowledge_list.size(0), knowledge_list.size(2)).cuda()   # [128,10]
    for batch_idx, (p, p_class, p_mask) in enumerate(zip(max_p, max_p_class, max_p_mask)):   # 4,4,4
    ...
    consensus_knowledge_conf, consensus_knowledge = consensus_knowledge.max(1)      # 128,128
    consensus_knowledge_mask = (max_conf > confidence_gate).float().cuda()          # 128
    consensus_knowledge = torch.zeros(consensus_knowledge.size(0), num_classes).cuda().scatter_(1,consensus_knowledge.view(-1, 1), 1)    # [128,10]
    # consensus_knowledge = torch.zeros(consensus_knowledge.size(0), num_classes).scatter_(1,consensus_knowledge.view(-1, 1), 1)
