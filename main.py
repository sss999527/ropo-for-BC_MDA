import math
#  python main.py --config DigitFive.yaml --target-domain syn -bp "E:/multi_source_DA/BCMDA"
import random
random.seed(1)
import numpy as np
...
# Dataset Parameters
parser.add_argument("--config", default="Office-Caltech10.yaml")
parser.add_argument('-bp', '--base_path', default="./")  # base_path
# parser.add_argument('--target-domain', type=str, help="The target domain we want to perform domain adaptation")
parser.add_argument('--target-domain', default="amazon" ,type=str, help="The target domain we want to perform domain adaptation")
...
# import config files
# gbk解码错误  with open(r"./config/{}".format(args.config)) as file:
with open(r"./config/{}".format(args.config), encoding='utf-8') as file:
...
def main(args=args, configs=configs):
    ...
    if configs["DataConfig"]["dataset"] == "DigitFive":
        ...
        ## models.append(CNN(args.data_parallel))
        ...
        classifiers.append(
            OfficeCaltechClassifier(configs["ModelConfig"]["backbone"], 10, args.data_parallel)
        )#.cuda()
        domains.remove(args.target_domain)
        args.source_domains = domains
        for domain in domains:
        ...
        models.append(
            OfficeCaltechNet(configs["ModelConfig"]["backbone"], bn_momentum=args.bn_momentum,
                             ...
                             data_parallel=args.data_parallel))#.cuda()
        classifiers.append(
            OfficeCaltechClassifier(configs["ModelConfig"]["backbone"], 31, args.data_parallel)
        )#.cuda()
        domains.remove(args.target_domain)
        args.source_domains = domains
        for domain in domains:
    ...
    for classifier_optimizer in classifier_optimizers:
        classifier_optimizer_schedulers.append(
            CosineAnnealingLR(classifier_optimizer, configs["TrainingConfig"]["total_epochs"],
                              eta_min=configs["TrainingConfig"]["learning_rate_end"]))
    # create the event to save log info 创建event保存日志信息
    ...
    # writer_log_dir = "./log/{}/runs/train_time:{}.{}".format(configs["DataConfig"]["dataset"],args.train_time,args.target_domain)
    writer_log_dir = ".{}.{}.{}.{}".format(configs["DataConfig"]["dataset"], args.train_time, args.target_domain, ".".join(args.source_domains))
    ...
    writer = SummaryWriter(comment=writer_log_dir)
    # begin train
    ...
    for epoch in range(args.start_epoch, total_epochs):
        domain_weight = train(train_dloaders, models, classifiers, optimizers,
                              ...
                              mix_aug=(configs["DataConfig"]["dataset"] != "AmazonReview"))  # 接收很多参数给train函数训练
        test(args.target_domain, args.source_domains, test_dloaders, models, classifiers, epoch,
             writer, num_classes=num_classes, top_5_accuracy=(num_classes > 10))  # 所以用mnist没有top5
...
def save_checkpoint(state, filename):
    ...
    #                                                     args.train_time)
    filefolder = "./{}/parameter/train_time{}".format(configs["DataConfig"]["dataset"],
                                                        args.train_time)
